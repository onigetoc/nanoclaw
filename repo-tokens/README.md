...
```yaml
- uses: qwibitai/eureclaw/repo-tokens@v1
  with:
    include: 'src/**/*.ts'
    exclude: 'src/**/*.test.ts'
```
...
The badge color reflects what percentage of an LLMs context window the codebase fills (context window size is configurable, defaults to 200k which is a common size for modern LLMs). Green for under 30%, yellow-green for 30%-50%, yellow for 50%-70%, red for 70%+.
