...
## Overview
Simplify EureClaw's initial setup by automatically registering the first chat as the 'main' group when a user sends their first message. This eliminates the need for manual JID entry and hardcoded configuration scripts.
...
- 1.2: User starts EureClaw with `npm start`
...
- New users can start using EureClaw in < 2 minutes (token + first message)
