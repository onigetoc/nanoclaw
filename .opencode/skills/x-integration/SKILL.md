---
name: x-integration
description: X (Twitter) integration for EureClaw. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
1. **EureClaw is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep eureclaw shows PID and exit code 0
```
...
| `EURECLAW_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/eureclaw.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into EureClaw, make the following modifications:
...
// Replace with:
default:
const handled = await handleXIpc(data, sourceWorkspace, isMain, DATA_DIR);
if (!handled) {
    logger.warn({ type: data.type }, 'Unknown IPC task type');
}
```
...
```typescript
    ...createXTools({ workspaceFolder, isMain })
```
...
All paths below are relative to project root (`EURECLAW_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep eureclaw  # Should show PID and exit code 0 or -
```
...
**Note:** Only the main workspace can use X tools. Other workspaces will receive an error.
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/eureclaw.log | tail -20

# Script errors
grep -i "error\|failed" logs/eureclaw.log | tail -20
```
...
container run eureclaw-agent ls -la /app/src/skills/
```
...
- Only main workspace can use X tools (enforced in `agent.ts` and `host.ts`)
