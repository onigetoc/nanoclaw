...
export const searchFiles = tool({
  ...
  args: {
    pattern: tool.schema.string().describe("Glob pattern to match (e.g., '*.ts')"),
  ...
  async execute({ pattern, directory, maxDepth }) {
    // Using Bun's shell
    // @ts-ignore for example only
    const { $ } = await import("bun")

    try {
      const result = await $`find ${directory} -maxdepth ${maxDepth} -name "${pattern}" -type f`.text()
