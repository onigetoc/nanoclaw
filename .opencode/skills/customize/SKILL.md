---
name: customize
description: Add new capabilities or modify EureClaw behavior. Use when user wants to add channels (Telegram, Slack, email input), change triggers, add integrations, modify the router, or make any other customizations. This is an interactive skill that asks questions to understand what the user wants.
---

# EureClaw Customization
...
| `workspaces/AGENTS.md` | Global memory/persona |
...
2. Document available tools in `workspaces/AGENTS.md`
...
Persona changes → edit `workspaces/AGENTS.md`
Per-workspace behavior → edit specific workspace's `AGENTS.md`
...
Implementation:
1. Commands are handled by the agent naturally — add instructions to `workspaces/AGENTS.md` or the workspace's `AGENTS.md`
2. For trigger-level routing changes, modify `processWorkspaceMessages()` in `src/index.ts`
