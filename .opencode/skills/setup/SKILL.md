...
If HAS_ENV=true from step 1, read `.env` and check if it already has `opencode_CODE_OAUTH_TOKEN` or `ANTHROPIC_API_KEY`. If so, confirm with user: "You already have opencode credentials configured. Want to keep them or reconfigure?" If keeping, skip to step 5.

AskUserQuestion: opencode subscription (Pro/Max) vs Anthropic API key?

**Subscription:** Tell the user:
1. Open another terminal and run: `opencode setup-token`
2. Copy the token it outputs
3. Add it to the `.env` file in the project root: `opencode_CODE_OAUTH_TOKEN=<token>`
