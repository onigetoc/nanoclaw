...
│  │    • mcp__eureclaw__* (scheduler tools via IPC)                │   │
│  │                                                                │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
...
| Agent | @anthropic-ai/opencode-sdk (0.2.29) | Run opencode with tools and MCP servers |
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'eureclaw-agent:latest';
...
```bash
opencode_CODE_OAUTH_TOKEN=sk-ant-oat01-...
```
...
- `{{PROJECT_ROOT}}` - Absolute path to your eureclaw installation
...
EureClaw uses a hierarchical memory system based on AGENTS.md files.
...
opencode: [calls mcp__eureclaw__schedule_task]
        {
...
### EureClaw MCP (built-in)

The `eureclaw` MCP server is created dynamically per agent call with the current group's context.
...
EureClaw runs as a single macOS launchd service.
...
### Service: com.eureclaw

**launchd/com.eureclaw.plist:**
```xml
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/eureclaw.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/eureclaw.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.eureclaw.plist ~/Library/LaunchAgents/
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep eureclaw` |
| "OpenCode process exited with code 1" | Apple Container failed to start | Check logs; EureClaw auto-starts container system but may fail |
...
- `logs/eureclaw.log` - stdout
- `logs/eureclaw.error.log` - stderr
