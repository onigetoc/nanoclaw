...
Findings from reverse-engineering `@anthropic-ai/opencode-sdk` v0.2.29–0.2.34 to understand how `query()` works, why agent teams subagents were being killed, and how to fix it. Supplemented with official SDK reference docs.
...
Agent Runner (our code)
  └── query() → SDK (sdk.mjs)
        └── spawns CLI subprocess (cli.js)
              └── opencode API calls, tool execution
...
Each invocation = one API call to opencode (one "turn").
...
| `additionalDirectories` | `string[]` | `[]` | Additional directories opencode can access |
...
| `model` | `string` | Default from CLI | opencode model to use |
| `outputFormat` | `{ type: 'json_schema', schema: JSONSchema }` | `undefined` | Structured output format |
| `pathToopencodeCodeExecutable` | `string` | Uses built-in | Path to OpenCode executable |
...
| `systemPrompt` | `string \| { type: 'preset'; preset: 'opencode_code'; append?: string }` | `undefined` | System prompt. Use preset to get OpenCode's prompt, with optional `append` |
| `tools` | `string[] \| { type: 'preset'; preset: 'opencode_code' }` | `undefined` | Tool configuration |
...
// 'project' → .opencode/settings.json (version controlled)
// 'local'   → .opencode/settings.local.json (gitignored)
```
...
| `assistant` | — | opencode's response (text + tool calls) |
...
opencode responded with text only — it decided it has completed the task. The API's `stop_reason` will be `"end_turn"`. The SDK does NOT make this decision — it's entirely driven by opencode's model output.
...
2. assistant/user       → opencode reasoning, tool calls, tool results
