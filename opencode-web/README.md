...
# Start opencode server (allow this UI origin)
cd /path/to/your-project
opencode serve --cors http://localhost:5189

# Start development server
bun dev
```

The application will be available at `http://localhost:5189`
...
- If you see `ERR_CONNECTION_REFUSED` on `127.0.0.1:4096`, the opencode server is not running on that port.
- Start it with:

```bash
opencode serve --hostname 127.0.0.1 --port 4096 --cors http://localhost:5174
```

- Important: the command is `opencode serve` (not `opencode server`).
